 ...
 */
static GLvoid
glmSecondPass(GLMmodel* model, FILE* file, GLuint* texIndices)
{
    GLuint numvertices;        /* number of vertices in model */
    GLuint numnormals;         /* number of normals in model */
    GLuint numtextures;         /* number of textures in model */
    ...
    GLfloat* textures;         /* array of vertices  */

    GLuint numindices = 0;
    GLuint numtindices = 0;
    ...
    allocated arrays */
    numvertices = numnormals = numtextures = 0;
    numtriangles = 0;
    ...
    while (fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
            ...
            case '\0':          /* vertex */
                ...
                break;
            case 'n':           /* normal */
                fscanf(file, "%f %f %f", &model->normals[numnormals].x, &model->normals[numnormals].y, &model->normals[numnormals].z);
                numnormals++;
                break;
            case 't':           /* textures */
                fscanf(file, "%f %f", &model->textures[numtextures].x, &model->textures[numtextures].y);
                numtextures++;
                break;
            }
            break;
    ...
    printf("Number of textures are: %d\n", numtextures);
...
glmReadOBJ(const char* filename)
{
    ...
    GLuint* cIndices = (GLuint*)malloc(sizeof(GLuint) * (model->numindices));

    /* rewind to beginning of file and read in the data this pass */
    rewind(file);

    glmSecondPass(model, file, tIndices);

    model->numnormals = model->numvertices;
    model->numtextures = model->numvertices;

    memcpy(cIndices, tIndices, sizeof(GLuint) * (model->numindices));
